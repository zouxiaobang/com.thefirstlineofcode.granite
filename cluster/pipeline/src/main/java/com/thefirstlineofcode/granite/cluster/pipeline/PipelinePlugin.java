...
public class PipelinePlugin extends Plugin implements IServerConfigurationAware, IRepositoryAware {
	...
	public PipelinePlugin(PluginWrapper wrapper) {
		super(wrapper);
	}
	...
	public void start() {
		...
		registerDeployClusterComponents(ignite, deployPlan);
	}
	...
	private void registerDeployClusterComponents(Ignite ignite, DeployPlan deployPlan) {
		repository.registerSingleton(Constants.COMPONENT_ID_IGNITE, startIgnite());
	...
	@Override
	public void stop() {
		removeDeployClusterComponents();
		Ignition.stop(true);
	}

	private void removeDeployClusterComponents() {
		repository.removeSingleton(Constants.COMPONENT_ID_IGNITE);
		repository.removeSingleton(Constants.COMPONENT_ID_NODE_RUNTIME_CONFIGURATION);
	}
