...
@Component("default.component.protocol.processing.processor")
public class DefaultProtocolProcessingProcessor implements com.firstlinecode.granite.framework.core.integration.IMessageProcessor,
	...
	private static final String CONFIGURATION_KEY_DELIVER_UNKNOWN_MESSAGE = "deliver.unknown.message";
	...
	private static final String KEY_GRANITE_COMPONENT_XEP_PROCESSORS = "Granite-Component-Xep-Processors";
	...
	private boolean deliverUnknownMessage;
	...
	private IXepProcessor<Stanza, Object> deliverToClientProcessor;
	...
	public synchronized void init() {
		OsgiUtils.trackContribution(bundleContext, KEY_GRANITE_COMPONENT_XEP_PROCESSORS, xepProcessorsTracker);
	}
	...
	public void setBundleContext(BundleContext bundleContext) {
		...
		deliverToClientProcessor = createDeliverToClientProcessor();
	}
	
	private IXepProcessor<Stanza, Object> createDeliverToClientProcessor() {
		IXepProcessor<Stanza, Object> xepProcessor;
		try {
			xepProcessor = new DeliverToClientProcessor();
		} catch (Exception e) {
			throw new RuntimeException("can't instantiate DeliverToClientProcessor", e);
		}
		
		appComponentService.inject(xepProcessor, bundleContext);
		return xepProcessor;
	}
	...
	public void process(IConnectionContext context, IMessage message) {
		try {
			...
			if (e instanceof UndeclaredThrowableException) {
				...
				if(pe != null) {
					processProtocolException(context, pe, message.getPayload());
					return;
				}
			}
	...
	private void processProtocolException(IConnectionContext context, ProtocolException e, Object message) {
		...
		if ((error instanceof Stanza) && (message instanceof Stanza)) {
	...
	private void doProcess(IProcessingContext context, IMessage message) throws RuntimeException {
		...
		if (object instanceof Stanza) {
			...
			processXep(context, stanza);
		} else {
			context.write(object);
		}
	}
	
	private <K extends Stanza, V> void processXep(IProcessingContext context, Stanza stanza) throws RuntimeException {
		if (stanza.getTo() == null || isToDomain(stanza.getTo()) || isToDomainAlias(stanza.getTo())) {
			doProcessXep(context, stanza);
		} else {
			deliverXep(context, stanza);
		}
	}
	...
	private void deliverXep(IProcessingContext context, Stanza stanza) {
		// TODO Server Rules for Handling XML Stanzas(rfc3920 11)
	...
	private <V, K extends Stanza> void doProcessXep(IProcessingContext context, Stanza stanza) {
		...
		if (stanza.getObjects().size() == 1) {
			...
			xepProcessor.process(context, (K)stanza, (V)stanza.getObjects());
		}
	}

	private boolean isUnknownMessageDeliverable(JabberId to) {
	...
	private class DeliverToClientProcessor implements IXepProcessor<Stanza, Object> {
		@Dependency("resources.service")
		private IResourcesService resourcesService;
		
		@Override
		public void process(IProcessingContext context, Stanza stanza, Object xep) {
			doProcess(context, stanza, xep);
		}
		
		private <T extends Stanza> void doProcess(IProcessingContext context, T stanza, Object xep) {
			JabberId to = stanza.getTo();
			
			if (!to.isBareId()) {
				T cloned = StanzaCloner.clone(stanza);
				cloned.setFrom(context.getJid());
				context.write(stanza);
			} else {
				for (IResource resource : resourcesService.getResources(to)) {
					if (!resource.isAvailable())
						continue;
					
					T cloned = StanzaCloner.clone(stanza);
					cloned.setFrom(context.getJid());
					cloned.setTo(resource.getJid());
					
					context.write(cloned);
				}
			}
			
			if (stanza instanceof Iq && stanza.getId() != null) {
				context.write(context.getJid(), new Iq(Iq.Type.RESULT, stanza.getId()));
			}
		}
		
	}

	private ProtocolChain getFirstProtocolChain(Stanza stanza) {
		return ProtocolChain.first(getStanzaProtocol(stanza)).next(stanza.getObjectProtocol(stanza.getObject().getClass()));
	}
	...
	public void setConfiguration(IConfiguration configuration) {
		stanzaErrorAttachSenderMessage = configuration.getBoolean(
			CONFIGURATION_KEY_STANZA_ERROR_ATTACH_SENDER_MESSAGE,
				false);
		deliverUnknownMessage = configuration.getBoolean(
			CONFIGURATION_KEY_DELIVER_UNKNOWN_MESSAGE,
				false);
	}
