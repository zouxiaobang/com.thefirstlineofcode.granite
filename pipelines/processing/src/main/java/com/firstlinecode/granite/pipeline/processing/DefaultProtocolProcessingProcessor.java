...
import com.firstlinecode.granite.framework.core.pipeline.processing.DefaultIqResultProcessor;
...
public class DefaultProtocolProcessingProcessor implements com.firstlinecode.granite.framework.core.pipeline.IMessageProcessor,
	...
	protected DefaultPresenceProcessor defaultPresenceProcessor;
	protected DefaultMessageProcessor defaultMessageProcessor;
	protected DefaultIqResultProcessor defaultIqResultProcessor;
	
	protected Map<ProtocolChain, IXepProcessor<?, ?>> singletonXepProcessores;
	...
	public DefaultProtocolProcessingProcessor() {
		...
		singletonXepProcessores = new HashMap<>();
	}
	...
	private void loadDefaultSimpleStanzaProcessors() {
		...
		appComponentService.inject(defaultMessageProcessor);
		
		defaultPresenceProcessor = new DefaultPresenceProcessor();
		appComponentService.inject(defaultPresenceProcessor);
		
		List<Class<? extends ISimpleStanzaProcessorsFactory>> tanzaProcessorFactoryClasses = appComponentService.getExtensionClasses(ISimpleStanzaProcessorsFactory.class);
		if (tanzaProcessorFactoryClasses == null || tanzaProcessorFactoryClasses.size() == 0) {
			logger.info("Simple stanza factorory not be found. XMPP IM protocol may not be supported");
			return;
		}
		
		if (tanzaProcessorFactoryClasses.size() != 1) {
			logger.error("Servral simple stanza factories have found. You may configure your plugins incorrectly. Please check your configuration.");
			throw new RuntimeException("Servral simple stanza factories have found. Please check your plugins.");
		}
		
		ISimpleStanzaProcessorsFactory stanzaProcessorsFactory = appComponentService.createExtension(tanzaProcessorFactoryClasses.get(0));
		...
		for (IMessageProcessor messageProcessor : stanzaProcessorsFactory.getMessageProcessors()) {
			defaultMessageProcessor.addMessageProcessor(messageProcessor);
		}
		for (IPresenceProcessor presenceProcessor : stanzaProcessorsFactory.getPresenceProcessors()) {
			defaultPresenceProcessor.addPresenceProcessor(presenceProcessor);
		}
	...
	private boolean processMessage(IProcessingContext context, Message message) {
		if (defaultMessageProcessor != null && defaultMessageProcessor.process(context, message)) {
			return true;
		}
	...
	private boolean processPresence(IProcessingContext context, Presence presence) {
		if (defaultPresenceProcessor != null) {
			return defaultPresenceProcessor.process(context, presence);
		}
	...
	private <K extends Stanza, V> IXepProcessor<K, V> getXepProcessor(ProtocolChain protocolChain) {
		IXepProcessor<K, V> xepProcessor = (IXepProcessor<K, V>)singletonXepProcessores.get(protocolChain);
